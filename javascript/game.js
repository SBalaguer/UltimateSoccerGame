class Game {
    ...
    if (this.gameStatus) {
      ...
      if (!this.goalScored) {
        ...
        if (this.timer2 < timestamp - this.speed2) {
          ...
          if (
            this.obstacleTimer < timestamp - this.obstacleSpeed &&
            !this.obstacles.obstacleEffect &&
            !this.obstacles.obstacleExists
          ) {
            this.obstacleTimer = timestamp;
            this.obstacles.obstacleCreation(
              this.obstacles.obstacleExists,
              this.obstacles.obstacleEffect
            );
  ...
  collision(power) {
    ...
    const D1 = Math.sqrt(
      (this.player1.PCX - (this.ball.BCX + this.ball.vx)) ** 2 +
        (this.player1.PCY - (this.ball.BCY + this.ball.vy)) ** 2
    );
    const R1 = this.player1.RADIUS + this.ball.RADIUS + COEFSEG;
    const D2 = Math.sqrt(
      (this.player2.PCX - (this.ball.BCX + this.ball.vx)) ** 2 +
        (this.player2.PCY - (this.ball.BCY + this.ball.vy)) ** 2
    );
  ...
  checkBallCollision() {
    ...
    if (
      this.ball.BCY + this.ball.RADIUS + this.ball.vy >= DL ||
      this.ball.BCY - this.ball.RADIUS + this.ball.vy <= TL
    ) {
    ...
    if (
      ...
      (this.ball.BCY - this.ball.RADIUS + this.ball.vy >= TL &&
        this.ball.BCY + this.ball.vy < SG &&
        this.ball.BCX - this.ball.RADIUS + this.ball.vx <= LL1) ||
      (this.ball.BCY + this.ball.vy > EG &&
        this.ball.BCY + this.ball.RADIUS + this.ball.vy <= DL &&
        this.ball.BCX - this.ball.RADIUS + this.ball.vx <= LL2)
    ) {
      this.ball.vx *= -1;
      this.player2.playerKicked = false;
    }
    if (
      ...
      (this.ball.BCY - this.ball.RADIUS + this.ball.vy >= TL &&
        this.ball.BCY + this.ball.vy < SG &&
        this.ball.BCX + this.ball.RADIUS + this.ball.vx >= RL1) ||
      (this.ball.BCY + this.ball.vy > EG &&
        this.ball.BCY + this.ball.RADIUS + this.ball.vy <= DL &&
        this.ball.BCX + this.ball.RADIUS + this.ball.vx >= RL2)
    ) {
      this.ball.vx *= -1;
      this.player1.playerKicked = false;
    }
  ...
  score() {
    ...
    if (
      ...
      (this.ball.BCY + this.ball.vy >= SG &&
        this.ball.BCY + this.ball.vy <= EG &&
        this.ball.BCX - this.ball.RADIUS + this.ball.vx >= RL1 + GS) ||
      (this.ball.BCY + this.ball.vy >= SG &&
        this.ball.BCY + this.ball.vy <= EG &&
        this.ball.BCX - this.ball.RADIUS + this.ball.vx >= RL2 + GS)
    ) {
      this.goalSound.play();
      this.score1 += 1;
      this.ball.vx = 0;
  ...
  collisionObstacles() {
    const D = Math.sqrt(
      (this.obstacles.x - (this.ball.BCX + this.ball.vx)) ** 2 +
        (this.obstacles.y - (this.ball.BCY + this.ball.vy)) ** 2
    );
